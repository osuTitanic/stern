...
def active_users_key(forum_id: int) -> str:
    return f"forum:{forum_id}:active"

def mark_user_active(user_id: int, forum_id: int) -> None:
    key = active_users_key(forum_id)
    now = int(time.time())

    with app.session.redis.pipeline(transaction=False) as pipe:
        pipe.zadd(key, {str(user_id): now})
        pipe.expire(key, FORUM_ACTIVITY_EXPIRY)
        pipe.execute()

def is_user_active(user_id: int, forum_id: int) -> bool:
    key = active_users_key(forum_id)
    last_active = app.session.redis.zscore(key, str(user_id))

    if last_active is None:
        return False

    if time.time() - float(last_active) >= FORUM_ACTIVITY_EXPIRY:
        app.session.redis.zrem(key, str(user_id))
        return False
...
def get_active_users(forum_id: int) -> list[int]:
    key = active_users_key(forum_id)
    cutoff = int(time.time()) - FORUM_ACTIVITY_EXPIRY

    with app.session.redis.pipeline(transaction=False) as pipe:
        pipe.zremrangebyscore(key, "-inf", cutoff)
        pipe.zrevrangebyscore(key, "+inf", f"({cutoff}")
        _, user_ids = pipe.execute()
