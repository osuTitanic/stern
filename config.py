...
DEFAULT_LOUNGE_BACKEND = f"http{'s' if ENABLE_SSL else ''}://lounge.{DOMAIN_NAME}"
...
LOUNGE_BACKEND = os.environ.get('LOUNGE_BACKEND', DEFAULT_LOUNGE_BACKEND)
...
SUPER_FRIENDLY_USERS = [int(i) for i in os.environ.get('SUPER_FRIENDLY_USERS', '').split(',') if i.isdigit()]
